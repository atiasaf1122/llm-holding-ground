...
def test_a_rounds_peer_blocks_are_a_pure_function_of_the_round_before_it() -> None:
    ...
    # Act
    blocks = rebuttal_peers(composition=composition, views=views)

    # Assert
    assert len(blocks) == composition.size
    assert blocks == rebuttal_peers(composition=composition, views=views)
...
def test_the_peer_block_claims_nothing_about_what_the_peers_read() -> None:
    ...
    composition = committee()
    blocks = rebuttal_peers(composition=composition, views=views_of(composition, marker="x"))
